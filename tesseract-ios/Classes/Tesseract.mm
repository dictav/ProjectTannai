...
- (void)setImageWithPixels:(const unsigned char *)pixels withSize:(CGSize)size bytesPerPixel:(int)bytesPerPixel
{
    _tesseract->SetImage(pixels, size.width, size.height, bytesPerPixel, size.width * bytesPerPixel);
}
